...
// Final working image update script - removes id and documentId fields
const API_BASE = 'http://localhost:1337/api';

// Helper function to remove problematic fields from objects
function cleanForStrapi(obj) {
  ...
  if (obj && typeof obj === 'object') {
    ...
    for (const [key, value] of Object.entries(obj)) {
      // Skip problematic fields that cause validation errors
...
async function updateLandingPageImages(apiToken) {
  try {
    ...
    console.log('📝 Updating landing page with cleaned image references...');
...
async function main(apiToken) {
  try {
    console.log('🚀 Starting final working image update (Cleaned Version)...');
    ...
    console.log('🔧 Removing id, documentId, and other problematic fields...');
...
if (!apiToken) {
  ...
  console.log('Run this script with: node update-images-clean.js YOUR_TOKEN_HERE');
  process.exit(1);
}
