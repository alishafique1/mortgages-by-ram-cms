...
// Authenticated Strapi API update script for landing page content
...
function showTokenInstructions() {
  console.log('\n🔑 API TOKEN REQUIRED');
  console.log('To update content via API, you need to create an API token in Strapi:');
  console.log('');
  ...
  console.log('6. Run this script with: node update-strapi-content-authenticated.js YOUR_TOKEN_HERE');
  console.log('');
  console.log('Alternatively, update content manually:');
  console.log('1. Go to: http://localhost:1337/admin');
  console.log('2. Navigate to: Content Manager → Single Types → Landing Page');
  console.log('3. Use the content from: quick-copy-paste-content.md');
}
...
async function updateLandingPageWithAuth(apiToken) {
  try {
    ...
    // Update using PUT method with authentication
    ...
    if (updateResponse.ok) {
      ...
      if (publishResponse.ok) {
        ...
        console.log('📱 Content should now be visible on the frontend!');
      } else {
